...
function insertarPropietarios() { // le pasamos los valores necesarios para insertar un registro
    ...
    if (mysqli_query($con, $query)) {
        ...
        echo "<h1>Se ha producido un error en la consulta. Compruebe que ha introducido correctamente su DNI y su dirección de correo electrónico</h1>";
...
function eliminarPropietarios() { 
    ...
    if (mysqli_query($con, $query)) {
        ...
        //var_dump($con); 
        echo "<h1>Se ha producido un error, no ha sido posible darse de baja</h1>";
...
function editarPropietarios($dni, $direccion, $cp, $ciudad, $pais, $email, $contrasena, $telefono) { // hay que pasarle las variables nombre y contraseña para identificar al usuario que vamos a eliminar
    ...
    if (mysqli_query($con, $query)) {
        ...
        $probando = mysqli_affected_rows($con); // comprueba que se ha modificado el registro
...
// ENVIAR CONTRASEÑA AL PROPIETARIO

function forgetPropietarios(){
    ...
    if ($element = mysqli_fetch_row($envioQuery)) {
        ...
        Has solicitado restablecer la contraseña de tu usuario en la web de TraveHome.

        <a href='index.php?nu=%207'>Pinche aquí</a>
        ...
        $newmensaje = wordwrap($mensaje, 130, "<br /><br />");
        
        if(mail($element[0], 'Confirmación de contraseña', $newmensaje)){
        ...
        //var_dump(mail($fila[0], 'Confirmación de contraseña', $newmensaje));
...
function reminderPropietarios($email, $contrasena) {
